...
const dismissGuard = Guard.fn<z.infer<typeof DismissSchema>, z.infer<typeof IncidentContextSchema>>(
  (ctx) =>
    (ctx.context.severity === 'P3' || ctx.context.severity === 'P4') && !ctx.context.isDataBreach,
);
...
const closeGuard = Guard.and([
  ...
  Guard.fn<z.infer<typeof CloseSchema>, z.infer<typeof IncidentContextSchema>>(
    (ctx) =>
      !ctx.context.isDataBreach || ctx.instanceState.isStateCompleted('stakeholders-confirmed'),
  ),
]);
...
export const incidentWorkflow = createWorkflow({ name: 'it-incident-response' })
  ...
  .addFork('investigation-fork', {
    label: 'Investigation Fork',
    targets: ['root-cause-analysis', 'stakeholders-notified'],
  })
  .addJoin('investigation-join', {
    label: 'Investigation Complete',
    requires: ['root-cause-documented', 'stakeholders-confirmed'],
    mode: 'all',
  })
  ...
  .addJoin('review-join', {
    ...
    requires: ['tech-review-complete', 'post-mortem-complete'],
    mode: 'all',
  })
  ...
  .addTransition({
    from: 'stakeholders-notified',
    to: 'stakeholders-confirmed',
    on: 'NOTIFY_STAKEHOLDERS',
  })
  ...
  .addTransition({
    ...
    to: 'tech-review-complete',
    on: 'COMPLETE_TECH_REVIEW',
  })
  ...
  .addTransition({ from: 'review-join', to: 'resolved', on: 'CLOSE', guard: closeGuard })
