...
export const releasePipelineWorkflow = createWorkflow({ name: 'release-pipeline' })
  ...
  .addFork('pre-checks-fork', {
    label: 'Pre-checks Fork',
    targets: [
  ...
  .addJoin('pre-checks-join', {
    label: 'Pre-checks Complete',
    requires: ['change-ticket-ok', 'design-ok', 'security-ok', 'dep-audit-ok', 'license-ok'],
    mode: 'all',
  })
  ...
  .addJoin('builds-join', {
    label: 'Builds Complete',
    requires: ['api-built', 'frontend-built', 'worker-built', 'infra-planned'],
    mode: 'all',
  })
  ...
  .addJoin('tests-join', {
    ...
    requires: ['unit-tests-ok', 'integration-tests-ok', 'e2e-tests-ok', 'load-tests-ok'],
    mode: 'all',
  })
  ...
  .addFork('staging-deploy-fork', {
    label: 'Staging Deploy Fork',
    targets: ['staging-api', 'staging-frontend', 'staging-worker'],
  })
  .addJoin('staging-deploy-join', {
    label: 'Staging Deploy Complete',
    requires: ['staging-api-up', 'staging-frontend-up', 'staging-worker-up'],
    mode: 'all',
  })
  ...
  .addFork('staging-validation-fork', {
    label: 'Staging Validation Fork',
    targets: ['staging-smoke', 'staging-regression', 'staging-performance'],
  })
  .addJoin('staging-validation-join', {
    label: 'Staging Validation Complete',
    requires: ['staging-smoke-ok', 'staging-regression-ok', 'staging-performance-ok'],
    mode: 'all',
  })
  ...
  .addFork('approvals-fork', {
    label: 'Approvals Fork',
    targets: ['engineering-approval', 'security-approval', 'product-approval'],
  })
  .addJoin('approvals-join', {
    label: 'All Approvals Received',
    requires: ['engineering-approved', 'security-approved', 'product-approved'],
    mode: 'all',
  })
  ...
  .addFork('prod-deploy-fork', {
    label: 'Prod Deploy Fork',
    targets: ['prod-eu', 'prod-us', 'prod-apac'],
  })
  .addJoin('prod-deploy-join', {
    label: 'Prod Deploy Complete',
    requires: ['prod-eu-up', 'prod-us-up', 'prod-apac-up'],
    mode: 'all',
  })
  ...
  .addFork('prod-validation-fork', {
    label: 'Prod Validation Fork',
    targets: ['prod-eu-smoke', 'prod-us-smoke', 'prod-apac-smoke'],
  })
  .addJoin('prod-validation-join', {
    label: 'Prod Validation Complete',
    requires: ['prod-eu-smoke-ok', 'prod-us-smoke-ok', 'prod-apac-smoke-ok'],
    mode: 'all',
  })
  ...
  .addTransition({
    from: 'unit-tests',
    to: 'unit-tests-ok',
    on: 'UNIT_TESTS_PASSED',
    guard: (ctx) => ctx.payload.failed === 0,
  })
  .addTransition({
    from: 'integration-tests',
    to: 'integration-tests-ok',
    on: 'INTEGRATION_TESTS_PASSED',
    guard: (ctx) => ctx.payload.failed === 0,
  })
  .addTransition({
    from: 'e2e-tests',
  ...
  .addTransition({
    from: 'load-tests',
    to: 'load-tests-ok',
    on: 'LOAD_TESTS_PASSED',
    guard: (ctx) =>
      ctx.payload.p99Ms < 200 && ctx.payload.p999Ms < 800 && ctx.payload.errorRate < 0.01,
  })
  ...
  .addTransition({
    from: 'staging-deploy-join',
    to: 'staging-validation-fork',
    on: 'START_STAGING_VALIDATION',
  })

  // Phase 5 — staging validation
  .addTransition({ from: 'staging-smoke', to: 'staging-smoke-ok', on: 'STAGING_SMOKE_OK' })
  .addTransition({
    from: 'staging-regression',
    to: 'staging-regression-ok',
    on: 'STAGING_REGRESSION_OK',
    guard: (ctx) => ctx.payload.failed === 0,
  })
  .addTransition({
    from: 'staging-performance',
    to: 'staging-performance-ok',
    on: 'STAGING_PERFORMANCE_OK',
  ...
  // Phase 6 — approvals
  .addTransition({
    from: 'engineering-approval',
    to: 'engineering-approved',
    on: 'ENGINEERING_APPROVED',
    guard: Guard.inject('engineering-director'),
  })
  .addTransition({
    from: 'security-approval',
    to: 'security-approved',
    on: 'SECURITY_APPROVED',
  ...
  .addTransition({
    from: 'prod-deploy-join',
    to: 'prod-validation-fork',
    on: 'START_PROD_VALIDATION',
  })
