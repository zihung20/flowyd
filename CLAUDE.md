...
- **Iconography sweep — emoji → icons (2026-06-06):** Removed all decorative emoji and Unicode-glyph "icons" across the docs site and web-runner UI in favour of proper icon components; no behaviour changes. `docs/index.md`: the six VitePress feature emoji (🔒 ✅ 🔀 ⏸️ 💾 📊) replaced with inline monochrome lucide SVG (rendered via VitePress's `v-html` icon slot). web-runner now uses `lucide-react` everywhere it previously used glyphs: `HomePage` feature cards (`ShieldCheck`/`GitFork`/`GitBranch`/`Workflow`), `DesignerPage` toolbar (`Sun`/`Moon` theme toggle, `Code2` Show Code, `Play` Run, `X` close), `ShowCodeModal` close (`X`), `DesignerToolbar` add-state buttons (`Square`/`GitFork`/`GitMerge`/`Pause`), `NodePanel` initial/terminal flags (`Play`/`Square`), `FlowNode` kind badges + initial/terminal markers, `RunnerToolbar` Live link (`ArrowUpRight`); `flowEdgeStyles` SVG edge labels dropped their glyph prefixes (now `auto`/`requires`). Typographic marks that are not emoji were intentionally kept: CTA arrows (`→`), section-divider box rules (`─`), and status checkmarks (`✓`) in ASCII diagrams / console output / CLI tooling. Root README documents the lucide-only iconography rule. web-runner lint + typecheck + build clean; `docs:build` clean.
- **web-runner payload history + time-travel scrubber (2026-06-06):** Three runner features, no flowyd library changes. (1) **Payload display** — `HistoryPanel` renders each `HistoryEntry.payload` in a per-row expand/collapse disclosure (`ChevronRight`/`ChevronDown`, monospace JSON), shown only when the payload is non-empty, so users see the exact value that drove every transition. (2) **Scrubber** (`TimelineBar`) — a video-style timeline under the graph: a range-slider playhead, a Play button that auto-advances frame by frame (the run "replay"), and a "Live" control to snap to the head. Scrubbing is **non-destructive**: the whole UI is a pure function of `useRunner().snapshot`, so `SingleRunner` swaps in `instRef.current.rewind(previewVersion)` (a pure detached read) for the displayed snapshot while `previewVersion !== null`; the live instance is never touched and the graph/history/form all time-travel for free. History rows are clickable to scrub and highlight the active version. (3) **Branch on dispatch** — dispatching while scrubbed commits a rewind from the playhead (replay-rebuild via `makeInstance()`, re-injecting guards, re-dispatching `history[0..version]`, discarding later steps); a `DynamicForm` banner warns first. `rewindTo` (destructive) is still available via each history row's `Undo2`. New context surface: `scrubTo` / `previewVersion` / `headVersion` / `isPreviewing` on `RunnerContextValue`; `rewind` added to the runner's `AnyInstance` shape (here and in `evaluateWorkflowCode.ts`). The runner has no `resolveWait` UI, so history only holds real dispatched actions — replay is deterministic. web-runner lint + typecheck + build clean.
- **Time-triggered transitions / deadlines (v0.29, 2026-06-06):** A transition is now triggered by **either** an action (`on`) **or** a deadline (`after`) — exactly one. `addTransition({ from, to, after: '48h' })` declares a timeout; the clock is anchored to `from` (starts on entry) and the edge fires scoped to `from` — never an action broadcast. `TransitionDefinition` is now itself a **discriminated union** on the trigger — `{ from; to; guard? } & TransitionTrigger`, where the shared `TransitionTrigger = ActionTrigger<OnT> | TimedTrigger<AfterT>` (each member forbids the other's key via `?: never`) is the single source of truth for the on-xor-after discriminant. Both the stored type and `addTransition`'s input shape (`TransitionInput`, a named type with per-branch guard payload typing — `TActions[K]` for actions, `unknown` for deadlines) compose from it, so the engine/builder/exporters **narrow** (`t.after !== undefined` → `after: number`; else → `on: string`) instead of guessing, dropping the `t.on as string` casts. New public type exports: `TransitionTrigger`, `ActionTrigger`, `TimedTrigger`. Compile-time "exactly one trigger" replaces the former runtime `build()` XOR check. Duration strings are normalised via new `parseDuration` in `core/utils.ts` (`ms`/`s`/`m`/`h`/`d`/`w`, decimals allowed). `build()` keeps only the load-bearing action-schema check (skipped for timed edges) — the on/after XOR and positive-`after` are enforced by the type, not re-checked at runtime; reachability/dead-end checks already treat timed edges as ordinary `from→to` arcs. The engine stays pure — new static `WorkflowEngine.fireTimed()` advances one timed edge through the existing `computeTransitions` fixed-point loop (so fork/join/auto-complete cascade for free), allowing the source to be `active` **or** `waiting` (a `WaitState` can time out). Timers are **derived from history** (no new snapshot field): `WorkflowInstance` gains `tick(now): Promise<number>` (fixed-point catch-up, each firing stamped with its logical `dueAt`, guard-blocked edges retried next tick) and `getNextDueAt(): string | null` (the one value a host indexes to drive a sweep). `dispatch` auto-advances elapsed deadlines to `now` before applying the action (skipped on `canExecute` dry-runs), so a stale action against an overdue state is correctly blocked; `restoreInstance` stays faithful (no mutation). **Clock injection:** the `WorkflowInstance` is the sole clock-reader — `dispatch(action, payload, { now? })`, `resolveWait(stateId, snap?, now?)`, and `createInstance(id, …, now?)` all accept an optional `now` defaulting to `new Date()`, and `WorkflowEngine.dispatch` now takes the timestamp as a parameter (`at`) so it never reads the clock and is genuinely pure/deterministic. `tick(now)` is the host pushing time forward; a live `dispatch` defaults `now` to real time but accepts an injected instant for deterministic replay/testing (the timeout suite needs no fake timers). **API consistency:** optional config now rides a trailing options object — `dispatch(action, payload, { now? })`, `resolveWait(stateId, { now? })`. The speculative `resolveWait` `externalSnapshot?: InstanceSnapshot` parameter was **removed** (wrong type — assumed the external process was a flowyd instance; and redundant — the action you dispatch next to leave the wait state already records its payload in the audit history). Updated `occ-disruption-sop` example + `api/workflow-instance`, `scenarios/external-wait`, `examples/disruption-sop` docs; dropped 2 tests that only exercised it. Fired deadlines record history `action: '__timeout:<from>-><to>'`, picked up unchanged by `rewind`; `getAvailableTransitions` skips timed edges. Visualization: `MermaidExporter` labels timed edges `after <duration>` (compact whole-unit via new `formatDuration`); `JsonGraphEdge` gains `after?: number` (present instead of `action`). New `tests/integration/timeout.test.ts` (15 cases: arming/next-due, fire vs no-fire, logical-time stamping, action-wins race, dispatch auto-advance after downtime, chained catch-up, branch give-up into a join, guarded-retry, rewind, duration parsing, build validation, exporters) + clock-injection coverage (`dispatch`/`resolveWait` stamp history `at` from an injected `now`; `createInstance` `now` anchors the initial-state deadline). 272 tests; full `lint && check:filemap && typecheck && test && build` gate clean.
- **Docs sync for deadlines + hooks (2026-06-06):** Brought the VitePress docs current with the library (no source change). Deadlines/timeouts had **zero** coverage and lifecycle hooks only appeared as `add*` option signatures. Added: two new scenario pages — `scenarios/timeouts.md` (deadline pattern, host-owns-clock `tick`/`getNextDueAt` sweep, catch-up/determinism, branch give-up, guarded deadlines) and `scenarios/hooks.md` (`onEnter`/`onExit` firing rules, `HookContext`, error propagation, code-not-data) — both wired into the sidebar (`config.ts`) and `scenarios/index.md`. Updated `api/workflow-builder.md` (`addTransition` `on`-xor-`after` union, `createInstance` `now`, new "Lifecycle hooks" section), `api/workflow-instance.md` (`dispatch` `{ now? }`, new `tick`/`getNextDueAt`), `guide/concepts.md` (Transitions → triggers, Deadlines + Lifecycle hooks subsections, snapshot `version` note), and the home page (`index.md`) with a "Deadlines & SLAs" feature card (lucide alarm-clock). `docs:build` clean; verified the hooks cross-page anchor resolves (renamed heading to parens so the slug matches).
