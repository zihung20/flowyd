...
### [examples rewrite — feature-complete learning ladder] (2026-06-07)
- **Replaced the three ad-hoc transit examples** (`engineer-predeparture-checklist`, `station-opening-checklist`, `occ-disruption-sop`) with a **numbered 7-example ladder** in `examples/` that collectively exercises every public API, plus an `examples/README.md` index (ladder table + feature-coverage map). Themes diversified (publishing, expense policy, peer review, support SLAs, lending, on-call incidents, no-code config) so the library reads as general-purpose, not transit-specific. `README.md` gained a "Runnable examples" section linking the ladder.
- **Tiers:** `01-document-approval-basics` (builder, Zod actions, steps, transitions, dispatch, blocked results, Mermaid) → `02-guards-and-context` (`setContext`, inline/`fn`/`and`/`or`/`not`/`inject` guards, guard-routing via mutually-exclusive guards, `canExecute`) → `03-parallel-fork-join` (fork/join, all three join modes incl. quorum, two-state auto-complete, `Guard.stateCompleted`/`or`) → `04-deadlines-hooks-wait` (`after` deadlines, `tick`/`getNextDueAt`, host clock, `onEnter`/`onExit`, `WaitState`/`resolveWait`) → `05-loan-origination` (flagship: context guards + fork/join + wait + deadline + hooks + snapshot/restore with guard re-injection + `rewind` + Mermaid & JSON exporters, service-layer style) → `06-incident-response` (role guards, multiple SLAs incl. a **deadline firing from a `waiting` state**, parallel mobilisation, persistence pattern, JSON-graph dashboard grouped by `StateKind`) → `07-dynamic-workflow` (`createDynamicWorkflow`: compile workflows from runtime config in loops, `build()` rejecting a malformed graph, an N-stage chain generated from an array).
- **API-honesty notes baked into the examples:** dynamic-workflow payloads are `unknown` so only `{}` is statically safe (the `Exact<>` helper marks provided keys `never`); `restoreInstance` after `JSON.parse` needs the definition's exact snapshot type (`Parameters<typeof wf.restoreInstance>[0]`), not the bare `string`-keyed shape. All seven typecheck under the project's strict config (examples are in `tsconfig` `include`) and run clean via `npx tsx`.
- Full `lint && check:filemap && typecheck && test && build` gate clean; 272 tests still pass; **no `src/` changes** (examples + README + CLAUDE only).

### [docs/examples overhaul — synced snippets, rendered diagrams, API page polish] (2026-06-07)
- **Docs now import the example source instead of copy-pasting it**, so the published example pages can never drift from what actually runs/typechecks. Each `examples/NN-*.ts` gained a `// #region example` … `// #endregion example` marker (wrapping everything below the imports); the doc pages embed that region with VitePress's `<<< ../../examples/NN-*.ts#example` snippet syntax. **Gotcha learned:** VitePress's `<<<` region regex (`/^\/\/ ?#?((?:end)?region) ([\w*-]+)$/`) requires the region **name on the `#endregion` line too** — a bare `// #endregion` silently fails `findRegion` and falls back to importing the whole file. The repo-relative import line (`../src/index.js`) sits *above* the region, so it's excluded; each doc page hand-writes the published `from 'flowyd'` / `from 'flowyd/visualization'` import (the only ~1–2 lines not snippet-sourced).
- **Replaced the four old example pages** (`approval-flow`, `parallel-inspection`, `disruption-sop`, `station-opening`) with seven matching the ladder (`basics`, `guards-and-context`, `parallel-fork-join`, `deadlines-hooks-wait`, `loan-origination`, `incident-response`, `dynamic-workflow`), each with a diagram + features list + snippet + "What to notice". Rewrote `docs/examples/index.md` (ladder table) and the `/examples/` sidebar in `.vitepress/config.ts`. No inbound links to the removed pages existed. README Documentation-table row updated (four → seven).
- Verified by `docs:build` (green, no dead links) + grepping the built HTML: region/endregion markers stripped, `../src/index.js` excluded, example bodies + public imports present. Full library gate (`lint && check:filemap && typecheck && test && build`) still clean; all 7 examples still run via `npx tsx`.
- **Auto-generated, drift-proof Mermaid diagrams.** Each example now `export`s its compiled workflow, with the demo run behind a `if (process.argv[1] === fileURLToPath(import.meta.url))` guard placed *outside* the `#region` so importing is side-effect-free (07 exports `onboardingWorkflow = compile(onboardingConfig)`). `scripts/gen-example-diagrams.ts` (run by `pnpm docs:diagrams`, which `docs:dev`/`docs:build` now invoke first) imports each workflow, runs `MermaidExporter.export(def)`, and writes `docs/examples/diagrams/<slug>.md` (a fenced ` ```mermaid ` partial). Each page shows the rendered diagram **first** via `<!--@include: ./diagrams/<slug>.md-->`; `vitepress-plugin-mermaid` renders it client-side (config wrapped in `withMermaid(...)`, `srcExclude: ['**/diagrams/*.md']` so partials aren't built as pages). Hand-drawn ASCII diagrams removed. New devDeps: `tsx`, `mermaid`, `vitepress-plugin-mermaid`. VitePress renders Mermaid but can't *derive* it from TS, so a generation step is required — this is that step.
- **pnpm + Mermaid dev-server gotcha.** `pnpm docs:dev` initially crashed in the browser with `dayjs.min.js … does not provide an export named 'default'`. Cause: `withMermaid` adds Mermaid's CommonJS deps (`dayjs`, `cytoscape`, `cytoscape-cose-bilkent`, `@braintree/sanitize-url`, `debug`) to Vite's `optimizeDeps.include`, but pnpm's strict `node_modules` doesn't hoist those transitive deps where Vite can resolve them (`Failed to resolve dependency: dayjs`), so they were never pre-bundled and the raw CJS file reached the browser. Fix: installed those five as direct devDependencies (resolve failures 6 → 0); `config.ts` also pins `vite.optimizeDeps.include`/`ssr.noExternal` for Mermaid. Production `docs:build` was never affected (Rollup handles CJS interop).
- **API page polish.** `api/state-types`: `StateKind`/`AnyState`/`describeState` moved under an "Advanced: inspecting a compiled workflow" section with a skip-note (most users never handle `AnyState`; you only do when iterating `getDefinition().states`). `api/workflow-instance`: renamed to just "WorkflowInstance" (sidebar + H1), added a grouped jump table, set `outline: { level: [2, 3] }` so method `h3`s appear in the page nav, and pinned a `{#dispatchresult}` anchor on the result section. `examples/index` "Running the examples" now points at the GitHub repo (clone) instead of assuming a local checkout.
