...
### [v0.15.0] 2026-05-28 — Required context at `createInstance` when schema is declared

`Workflow.createInstance` now uses a conditional rest-params signature: `context` is **required** when `setContext()` was called on the builder (`TContext` is concrete), and **optional** when no schema was declared (`TContext` remains `unknown`). Implemented via `unknown extends TContext ? [context?: TContext] : [context: TContext]`. Added two new compile-time type tests confirming the required/optional split; updated existing tests that called `createInstance` without context on schema-bearing workflows to supply a valid initial value. 196 tests; all pipeline steps clean.
