...
- **web-runner payload history + rewind (2026-06-06):** Two runner features, no flowyd library changes. (1) **Payload display** — `HistoryPanel` now renders each `HistoryEntry.payload` in a per-row expand/collapse disclosure (`ChevronRight`/`ChevronDown`, monospace JSON), shown only when the payload is non-empty, so users can see the exact value that drove every transition. (2) **Rewind** — clicking a past step's `Undo2` affordance rewinds the live run to that version and lets the user branch off and dispatch differently. Because `WorkflowInstance.rewind()` is a pure read (returns a detached snapshot, never mutates), rewind is implemented by **replay**: `SingleRunner.rewindTo(version)` rebuilds via the existing `makeInstance()` factory (which re-injects guards) and re-dispatches the recorded `(action, payload)` pairs for `history[0..version]`, discarding later history; divergence (a guard now blocking) is surfaced via `lastError`. The runner has no `resolveWait` UI, so history only ever holds real dispatched actions — replay is deterministic. New `rewindTo` added to `RunnerContextValue`. web-runner lint + typecheck + build clean.
